...
class Webp_Gallery_Video_Galleries {
	...
	public function load_video_gallery_page() {
		global $wpdb;
		$task = webp_gallery_video_get_video_gallery_task();
		...
		switch ( $task ) {
			case 'edit_cat':
				if ( $id ) {
					...
					$id = $wpdb->get_var( "SELECT MAX( id ) FROM " . $wpdb->prefix . "webpacegallery_gallerys" );
					$this->edit_video_gallery( $id );
				}
				break;
			case 'apply':
			...
			case 'remove_gallery_video':
				if ( ! isset( $_REQUEST['webpace_gallery_video_nonce_remove_video_gallery'] ) | ! wp_verify_nonce( $_REQUEST['webpace_gallery_video_nonce_remove_video_gallery'], 'webpace_gallery_video_nonce_remove_video_gallery' . $id ) ) {
					wp_die( 'Security check fail' );
				}
	...
	public function show_video_galleries_page() {
		...
		if ( $where ) {
			if ( $cat_id ) {
                ...
                $query  = $wpdb->prepare("SELECT COUNT(*) FROM " . $wpdb->prefix . "webpace_videogallery_galleries  WHERE name LIKE %s  AND sl_width=%d" ,'%'.$search_tag.'%',$cat_id);
           }
		} else {
			if ( $cat_id ) {
				$where .= " WHERE sl_width=" . $cat_id;
                $query  = $wpdb->prepare("SELECT COUNT(*) FROM " . $wpdb->prefix . "webpace_videogallery_galleries  WHERE sl_width=%d" ,$cat_id);
            }
		}
		$cat_row_query    = $wpdb->prepare("SELECT id,name FROM " . $wpdb->prefix . "webpace_videogallery_galleries WHERE sl_width= %d",0);
		...
		if ( $cat_id ) {
			$query = "SELECT  a.* ,  COUNT(b.id) AS count, g.par_name AS par_name FROM " . $wpdb->prefix . "webpace_videogallery_galleries  AS a LEFT JOIN " . $wpdb->prefix . "webpace_videogallery_galleries AS b ON a.id = b.sl_width LEFT JOIN (SELECT  " . $wpdb->prefix . "webpace_videogallery_galleries.ordering as ordering," . $wpdb->prefix . "webpace_videogallery_galleries.id AS id, COUNT( " . $wpdb->prefix . "webpace_videogallery_videos.videogallery_id ) AS prod_count
FROM " . $wpdb->prefix . "webpace_videogallery_videos, " . $wpdb->prefix . "webpace_videogallery_galleries
WHERE " . $wpdb->prefix . "webpace_videogallery_videos.videogallery_id = " . $wpdb->prefix . "webpace_videogallery_galleries.id
GROUP BY " . $wpdb->prefix . "webpace_videogallery_videos.videogallery_id) AS c ON c.id = a.id LEFT JOIN
(SELECT " . $wpdb->prefix . "webpace_videogallery_galleries.name AS par_name," . $wpdb->prefix . "webpace_videogallery_galleries.id FROM " . $wpdb->prefix . "webpace_videogallery_galleries) AS g
...
(SELECT " . $wpdb->prefix . "webpace_videogallery_galleries.name AS par_name," . $wpdb->prefix . "webpace_videogallery_galleries.id FROM " . $wpdb->prefix . "webpace_videogallery_galleries) AS g
 ON a.sl_width=g.id WHERE a.name LIKE '%" . $search_tag . "%'  group by a.id " . $order . " " . " LIMIT " . $limit . ",20";
		...
		$rows      = webp_gallery_video_open_cat_in_tree( $rows );
		$query     = "SELECT  " . $wpdb->prefix . "webpace_videogallery_galleries.ordering," . $wpdb->prefix . "webpace_videogallery_galleries.id, COUNT( " . $wpdb->prefix . "webpace_videogallery_videos.videogallery_id ) AS prod_count
FROM " . $wpdb->prefix . "webpace_videogallery_videos, " . $wpdb->prefix . "webpace_videogallery_galleries
WHERE " . $wpdb->prefix . "webpace_videogallery_videos.videogallery_id = " . $wpdb->prefix . "webpace_videogallery_galleries.id
GROUP BY " . $wpdb->prefix . "webpace_videogallery_videos.videogallery_id ";
		...
		$cat_row = webp_gallery_video_open_cat_in_tree( $cat_row );
		require_once( GALLERY_VIDEO_TEMPLATES_PATH . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'webp-video-admin-video-galleries-list.php' );
	}
	...
	public function edit_video_gallery( $id ) {
		...
		$query                          = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "webpace_videogallery_videos where videogallery_id = %d order by ordering ASC  ", $row->id );
		$rowim                          = $wpdb->get_results( $query );
		$query                          = "SELECT * FROM " . $wpdb->prefix . "webpace_videogallery_galleries order by id ASC";
		$rowsld                         = $wpdb->get_results( $query );
		$gallery_video_edit_video_nonce = wp_create_nonce( 'gallery_video_edit_video_nonce' );
		require_once( GALLERY_VIDEO_TEMPLATES_PATH . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'webp-video-gallery-admin-videos-list-view.php' );
	}
	...
	function save_video_gallery_data( $id ) {
		global $wpdb;
		$cat_row    = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "webpace_videogallery_galleries WHERE id!= %d ", $id ) );
		$query      = $wpdb->prepare( "SELECT sl_width FROM " . $wpdb->prefix . "webpace_videogallery_galleries WHERE id = %d", $id );
		...
		if ( isset( $_POST["name"] ) ) {
			...
			$webpace_sl_effects          = absint( $_POST["webpace_sl_effects"] );
			...
			if ( $_POST["name"] != '' ) {
				...
				$wpdb->update(
					...
					array(
						...
						'webpace_sl_effects'          => $webpace_sl_effects,
	...
	function remove_video_gallery( $id ) {
		...
		$wpdb->delete(
			$table_name,
			array( 'id' => $id ) );
		$sql_remov_tag = $wpdb->prepare( "DELETE FROM " . $wpdb->prefix . "webpace_videogallery_galleries WHERE id = %d", $id );
