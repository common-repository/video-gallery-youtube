 ...
 * @return bool
 */
function webp_issetTableColumn($table_name, $column_name)
{
    global $wpdb;
 ...
 * @return int
 */
function webp_gallery_video_get_video_gallery_id()
{
 ...
 * @return string
 */
function webp_gallery_video_get_video_gallery_task()
{
    if (isset($_GET['page']) && $_GET['page'] == 'video_galleries_webpace_video_gallery') {
        if (isset($_GET["task"])) {
 ...
 * @return array
 */
function webp_gallery_video_open_cat_in_tree($catt, $tree_problem = '', $hihiih = 1)
{
    global $wpdb;
...
(SELECT " . $wpdb->prefix . "webpace_videogallery_galleries.name AS par_name," . $wpdb->prefix . "webpace_videogallery_galleries.id FROM " . $wpdb->prefix . "webpace_videogallery_galleries) AS g
 ON a.sl_width=g.id WHERE a.name LIKE '%" . $search_tag . "%' AND a.sl_width=" . $local_cat->id . " group by a.id  " . $glob_ordering_in_cat;
        ...
        webp_gallery_video_open_cat_in_tree($new_cat, $tree_problem . "— ", 0);
    }
    return $trr_cat;
}

function webp_gallery_video_print_html_nav($count_items, $page_number, $serch_fields = "")
{
    ?>
