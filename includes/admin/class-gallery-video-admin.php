...
class Webp_Gallery_Video_Admin {
	...
	/**
	 * Instance of Webp_Gallery_Video_General_Options class
	 *
	 * @var Webp_Gallery_Video_General_Options
	 */
	public $general_options = null;

	/**
	 * Instance of Webp_Gallery_Video_Galleries class
	 *
	 * @var Webp_Gallery_Video_Galleries
	 */
	public $video_galleries = null;

	/**
	 * Instance of Webp_Gallery_Video_Lightbox_Options class
	 *
	 * @var Webp_Gallery_Video_Lightbox_Options
	 */
	public $lightbox_options = null;

	/**
	 * Instance of Webp_Gallery_Video_Featured_Plugins class
	 *
	 * @var Webp_Gallery_Video_Featured_Plugins
	 */
	...
	/**
	 * Webp_Gallery_Video_Admin constructor.
	 */
	...
	protected function init() {
		$this->general_options  = new Webp_Gallery_Video_General_Options();
		$this->video_galleries  = new Webp_Gallery_Video_Galleries();
		$this->lightbox_options = new Webp_Gallery_Video_Lightbox_Options();
	}
	...
	public function admin_menu() {
		$this->pages[] = add_menu_page( __( 'Video Gallery', 'gallery-video' ), __( 'Video Gallery', 'gallery-video' ), 'delete_pages', 'video_galleries_webpace_video_gallery', array(
			Webp_Gallery_Video()->admin->video_galleries,
			'load_video_gallery_page'
		), GALLERY_VIDEO_IMAGES_URL . "/admin_images/video_gallery_icon.png" );
		$this->pages[] = add_submenu_page( 'video_galleries_webpace_video_gallery', __( 'Video Galleries', 'gallery-video' ), __( 'Video Galleries', 'gallery-video' ), 'delete_pages', 'video_galleries_webpace_video_gallery', array(
			Webp_Gallery_Video()->admin->video_galleries,
			'load_video_gallery_page'
		) );

		$this->pages[] = add_submenu_page( 'video_galleries_webpace_video_gallery', __( 'Advanced Options', 'gallery-video' ), __( 'Advanced Options', 'gallery-video' ), 'delete_pages', 'Options_video_gallery_styles', array(
			Webp_Gallery_Video()->admin->general_options,
			'load_page'
		) );

		$this->pages[] = add_submenu_page( 'video_galleries_webpace_video_gallery', __( 'Lightbox Options', 'gallery-video' ), __( 'Lightbox Options', 'gallery-video' ), 'delete_pages', 'Options_video_gallery_lightbox_styles', array(
			Webp_Gallery_Video()->admin->lightbox_options,
			'load_page'
		) );
	...
	public function wp_loaded() {
		if ( isset( $_GET['page'] ) && $_GET['page'] == 'video_galleries_webpace_video_gallery' ) {
			if ( webp_gallery_video_get_video_gallery_task() ) {
				if ( webp_gallery_video_get_video_gallery_task() == 'add_cat' ) {
					if ( ! isset( $_REQUEST['webpace_gallery_video_nonce_add_gallery_video'] ) || ! wp_verify_nonce( $_REQUEST['webpace_gallery_video_nonce_add_gallery_video'], 'webpace_gallery_video_nonce_add_gallery_video' ) ) {
					...
					$wpdb->insert(
						...
						array(
							...
							'ordering'                    => 1,
							'published'                   => 300,
							'webpace_sl_effects'          => 4
						)
					);
					$apply_video_gallery_safe_link = wp_nonce_url(
						'admin.php?page=video_galleries_webpace_video_gallery&id=' . $wpdb->insert_id . '&task=apply',
	...
	public function wp_loaded_add_video() {
		...
		if ( isset( $_GET['page'] ) && $_GET['page'] == 'video_galleries_webpace_video_gallery' ) {
			if ( webp_gallery_video_get_video_gallery_task() && webp_gallery_video_get_video_gallery_id() ) {
				if ( webp_gallery_video_get_video_gallery_task() == 'videogallery_video' && $_GET['closepop'] == 1 ) {
					if ( ! isset( $_REQUEST['video_add_nonce'] ) || ! wp_verify_nonce( $_REQUEST['video_add_nonce'], 'webpace_gallery_nonce_add_video' ) ) {
						wp_die( 'Security check fail' );
					}
					$id = webp_gallery_video_get_video_gallery_id();
					global $wpdb;
					$title       = wp_kses( wp_unslash( $_POST["show_title"] ), 'default' );
					$description = wp_kses( wp_unslash( $_POST["show_description"] ), 'default' );
					$video_url   = sanitize_text_field( $_POST["webpace_add_video_input"] );
					$link_url    = sanitize_text_field( $_POST["show_url"] );
					if ( isset( $_POST["webpace_add_video_input"] ) ) {
						if ( $_POST["webpace_add_video_input"] != '' ) {
							$table_name   = $wpdb->prefix . "webpace_videogallery_videos";
							$query        = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "webpace_videogallery_galleries WHERE id= %d", $id );
							$row          = $wpdb->get_row( $query );
							$query        = $wpdb->prepare( "SELECT * FROM " . $wpdb->prefix . "webpace_videogallery_videos where videogallery_id = %d order by id ASC", $row->id );
					...
					$apply_video_gallery_safe_link = wp_nonce_url(
						'admin.php?page=video_galleries_webpace_video_gallery&id=' . $id . '&task=apply',
	...
	public function wp_loaded_edit_video() {
		if ( isset( $_GET['page'] ) && $_GET['page'] == 'video_galleries_webpace_video_gallery' ) {
			if ( webp_gallery_video_get_video_gallery_task() && webp_gallery_video_get_video_gallery_task() == 'gallery_video_edit_video' && $_GET['closepop'] == 1 ) {
				...
				$apply_video_gallery_safe_link = wp_nonce_url(
					'admin.php?page=video_galleries_webpace_video_gallery&id=' . $gallery_video_id . '&task=apply',
	...
	function wp_loaded_duplicate_video() {
		if ( isset( $_GET['page'] ) && $_GET['page'] == 'video_galleries_webpace_video_gallery' ) {
			if ( webp_gallery_video_get_video_gallery_task() ) {
				if ( webp_gallery_video_get_video_gallery_task() == 'duplicate_gallery_video' ) {
                    if ( !isset( $_GET["id"] ) || absint( $_GET['id'] ) != $_GET['id'] ) {
                        wp_die('"id" parameter is required to be not negative integer');
                    }
                    $id = absint( $_GET["id"] );
					if ( ! isset( $_REQUEST['gallery_video_duplicate_nonce'] ) || ! wp_verify_nonce( $_REQUEST['gallery_video_duplicate_nonce'], 'webpace_gallery_video_nonce_duplicate_gallery' . $id) ) {
						wp_die( 'Security check fail' );
					}
					global $wpdb;
					$table_name    = $wpdb->prefix . "webpace_videogallery_galleries";
					...
					$wpdb->insert(
						...
						array(
							...
							'webpace_sl_effects'          => $gallery_video[0]->webpace_sl_effects,
					...
					$query    = $wpdb->prepare( "SELECT id FROM " . $wpdb->prefix . "webpace_videogallery_galleries order by id ASC");
					...
					VALUES " . $videos_list ;
					$wpdb->query( $query);
					wp_redirect( 'admin.php?page=video_galleries_webpace_video_gallery' );
				}
			}
		}
	}
