 ...
 */
function webp_gallery_video_get_default_general_options() {
	$video_gallery_default__params = array(
		...
		'webp_gallery_video_version'                                          => '2.0.4',
        //new options
 ...
 * @return string
 */
function webp_gallery_video_get_view_slag_by_id( $id ) {
	global $wpdb;
	$query = $wpdb->prepare( "SELECT webpace_sl_effects FROM " . $wpdb->prefix . "webpace_videogallery_galleries WHERE id=%d", $id );
 ...
 * @return mixed
 */
function webp_gallery_video_get_ip() {
	if ( ! empty( $_SERVER['HTTP_CLIENT_IP'] ) ) {
		$webpace_ip = $_SERVER['HTTP_CLIENT_IP'];
	} elseif ( ! empty( $_SERVER['HTTP_X_FORWARDED_FOR'] ) ) {
		$webpace_ip = $_SERVER['HTTP_X_FORWARDED_FOR'];
	} else {
		$webpace_ip = $_SERVER['REMOTE_ADDR'];
	}
