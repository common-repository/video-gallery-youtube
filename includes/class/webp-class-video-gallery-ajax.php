...
class Webp_Gallery_Video_Ajax
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_nopriv_webpace_gallery_video_front_end_ajax', array(
            $this,
            'webpace_video_gallery_ajax_front_end_callback'
        ));
        add_action('wp_ajax_webpace_gallery_video_front_end_ajax', array(
            $this,
            'webpace_video_gallery_ajax_front_end_callback'
        ));
    ...
    function gallery_video_admin_ajax_shortecode_callback()
    {
        ...
        if ($_POST['post'] == 'video_gal_change_options') {
            ...
            $response = array(
                'webpace_sl_effects' => $row->webpace_sl_effects,
        ...
        if ($_POST['post'] == 'videoGalSaveOptions') {
            ...
            $view = absint($_POST["webpace_sl_effects"]);
    ...
    function gallery_video_admin_ajax_callback()
    {
        if (isset($_POST['task']) && $_POST['task'] == 'send_url_popup') {
            ...
            $video = webp_gallery_video_get_video_id_from_url(esc_url($_POST['video_url']));
            $video_id = $video[0];
            $video_type = $video[1];
            $response = array(
                'video_id' => $video_id,
    ...
    function webpace_video_gallery_ajax_front_end_callback()
    {
        $gallery_video_get_option = webp_gallery_video_get_default_general_options();
        ...
        if (isset($_POST['task']) && $_POST['task'] == "load_videos_content") {
            ...
            if (!empty($_POST["page"]) && is_numeric($_POST['page']) && $_POST['page'] > 0) {
                ...
                $start = $page * $num - $num;
                $query = $wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "webpace_videogallery_videos where videogallery_id = '%d' order by ordering ASC LIMIT %d,%d", $gallery_video_id, $start, $num);
                ...
                foreach ($page_videos as $key => $row) {
                    ...
                    $videourl = webp_gallery_video_get_video_id_from_url($row->image_url);
                    ...
                    $output .= '<div class="video-element_' . $gallery_video_id . ' video-element ' . $no_video_title . '" id="webpace_videogallery_pupup_element_' . $row->id . '_child" tabindex="0" data-symbol="' . $video_name . '"  data-category="alkaline-earth">';
...
<li class="pupup-element" id="webpace_videogallery_pupup_element_' . $row->id . '">
            ...
            if (!empty($_POST["page"]) && is_numeric($_POST['page']) && $_POST['page'] > 0) {
                ...
                foreach ($page_videos as $key => $row) {
                    ...
                    if ($videourl[1] == 'youtube') {
                        ...
                        $video = '<a class="vyoutube webpace_videogallery_item group1"  href="//www.youtube.com/embed/' . $videourl[0] . '" title="' . $video_name . '">
                        ...
                        $video = '<a class="vvimeo webpace_videogallery_item group1" href="http://player.vimeo.com/video/' . $videourl[0] . '" title="' . $video_name . '">
                                    <img src="' . $imgsrc . '" alt="" />
                                    <div class="play-icon ' . $videourl[1] . '-icon"></div>
                                 </a>';
                    }
            ...
            if (!empty($_POST["page"]) && is_numeric($_POST['page']) && $_POST['page'] > 0) {
                ...
                foreach ($page_videos as $key => $row) {
                    ...
                    if ($videourl[1] == 'youtube') {
                        ...
                        $video = '<a  class="vyoutube webpace_videogallery_item group1"  href="//www.youtube.com/embed/' . $videourl[0] . '" title="' . $video_name . '"></a>
                                    <img src="' . $thumb_pic . '" alt="' . $video_name . '" />';
                    } else {
                        ...
                        $video = '<a  class="vvimeo webpace_videogallery_item group1" href="http://player.vimeo.com/video/' . $videourl[0] . '" title="' . $video_name . '"></a>
                                    <img src="' . $imgsrc . '" alt="' . $video_name . '" />';
                    }
                    $icon = webp_gallery_video_youtube_or_vimeo($row->image_url);
                    if ($video_thumb != '') {
    ...
    function share_count_views_ajax_callback_function()
    {
        ...
        if (isset($_POST['task'])) {
            if ($_POST['task'] == "data-id") {
                if (webp_gallery_video_get_ip() != 'UNKNOWN') {
                    $ip = webp_gallery_video_get_ip();
                    if ( !isset( $_POST["id"] ) || absint( $_POST['id'] ) != $_POST['id'] ) {
                        echo json_encode(array('success'=>'0','message'=>'"id" parameter is required to be not negative integer'));
                    }
                    $vid_id = absint($_POST['id']);
                    $sql_web_result = $wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "webpace_videogallery_params_ip WHERE ip=%s and video_id=%d", $ip, $vid_id);
                    $ipcount = $wpdb->get_results($sql_web_result);
                    $ipcount = count($ipcount);
                    if ($ipcount == 0) {
                        $sql_web_result_view = $wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "webpace_videogallery_params_ip WHERE id=%d", $vid_id);
                        $view_count = $wpdb->get_results($sql_web_result_view);
                        $current_date = date("Y-m-d H:i:s");
                        $sql_webpace_insert = "INSERT INTO " . $wpdb->prefix . "webpace_videogallery_params_ip (video_id, ip, date) VALUES ('" . $vid_id . "','" . webp_gallery_video_get_ip() . "','" . $current_date . "')";
                        $wpdb->query($sql_webpace_insert);
