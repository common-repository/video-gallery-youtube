...
class Webp_Gallery_Video_Template_Loader
{
    /**
     * @param $template_name
    ...
    public static function locate_template( $template_name, $template_path = '', $default_path = '' ) {
        if ( ! $template_path ) {
            $template_path = Webp_Gallery_Video()->template_path();
        }
        if ( ! $default_path ) {
            $default_path = Webp_Gallery_Video()->plugin_path() . '/templates/';
        }
        /**
    ...
    public function load_front_end($videos, $gallery_video_get_option, $gallery_video)
    {
        ...
        global $wpdb, $post;
        $view = $gallery_video[0]->webpace_sl_effects;
        $arrowfolder = GALLERY_VIDEO_IMAGES_URL . '/arrows';
        $protocol = stripos($_SERVER['SERVER_PROTOCOL'], 'https') === true ? 'https://' : 'http://';
        $view_slug = webp_gallery_video_get_view_slag_by_id($gallery_videoID);
        ...
        $start = $page * $num - $num;
        $query = $wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "webpace_videogallery_videos where videogallery_id = '%d' order by ordering ASC LIMIT " . $start . "," . $num, $gallery_videoID);
