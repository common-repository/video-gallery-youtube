...
global $wpdb;
$webpace_gallery_video_nonce_add_gallery_video = wp_create_nonce( 'webpace_gallery_video_nonce_add_gallery_video' );
$protocol                                      = stripos( $_SERVER['SERVER_PROTOCOL'], 'https' ) === true ? 'https://' : 'http://';
$add_video_nonce                               = wp_create_nonce( 'webpace_gallery_nonce_add_video' );
?>

	<div class="wrap">
		<?php require( GALLERY_VIDEO_TEMPLATES_PATH . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'webp-video-gallery-admin-banner.php' ); ?>
		<?php
		...
		<form action="admin.php?page=video_galleries_webpace_video_gallery&id=<?php echo $row->id; ?>&save_data_nonce=<?php echo esc_attr($gallery_video_save_data_nonce);?>" method="post"
					...
					<ul id="videogallerys-list">
						...
						foreach ( $rowsld as $rowsldires ) {
							if ( $rowsldires->id != $row->id ) {
								...
								<li>
									<a href="#"
									   onclick="window.location.href='admin.php?page=video_galleries_webpace_video_gallery&task=edit_cat&id=<?php echo esc_attr($rowsldires->id); ?>'"><?php echo $rowsldires->name; ?></a>
								</li>
								<?php
							} else { ?>
								<li class="active"
						...
						?>
						<li class="add-new">
							<a onclick="window.location.href='admin.php?page=video_galleries_webpace_video_gallery&amp;task=add_cat&webpace_gallery_video_nonce_add_gallery_video=<?php echo esc_attr($webpace_gallery_video_nonce_add_gallery_video); ?>'">+</a>
						</li>
					</ul>
				</div>
					...
					<div id="post-body-content">
						...
						<div id="post-body">
							<div id="post-body-heading">
								<h3>Videos</h3>
								<a href="#TB_inline?width=700&height=500&inlineId=webpace_gallery_video_add_videos"
											...
											$i ++; ?> >
												...
												<div class="image-container">
													...
													<div>
														...
														<div
															class="webpace-editnewuploader uploader button<?php echo $rowimages->id; ?> add-new-image">
															<input type="button"
																...
																<div id="html_videogallery_video_test"
																	...
																	<div class="hg_pop_up_div">
																		...
																		<a class="get_full_version_pdf"
																		   href="https://webpace.net/wordpress-video-gallery/"
																		   target="_blank">
												...
												<div class="image-options">
													...
													<div class="description-block">
														...
														<textarea class="textareaadminpage" id="im_description<?php echo $rowimages->id; ?>"
													...
													<div class="remove-image-container">
														<a class="button remove-image"
														   href="admin.php?page=video_galleries_webpace_video_gallery&task=apply&id=<?php echo $row->id; ?>&remove_video=<?php echo $rowimages->id; ?>&save_data_nonce=<?php echo $gallery_video_nonce_remove_video; ?>">Remove
															Video</a>
														<a href="#TB_inline?width=700&height=500&inlineId=webpace_gallery_video_edit_videos"
						...
						<div id="side-sortables" class="meta-box-sortables ui-sortable">
							<div id="videogallery-unique-options" class="postbox">
								...
								<ul id="videogallery-unique-options-list">
									<li>
										<label for="webpace_videogallery_name"><?php _e('Gallery Name', 'gallery-video');?></label>
										<input type="text" name="name" id="webpace_videogallery_name"
										       value="<?php echo esc_html( stripslashes( $row->name ) ); ?>"
										       onkeyup="name_changeRight(this)">
									</li>
									<li>
										<label for="webpace_sl_effects"><?php _e('Views', 'gallery-video');?></label>
										<select name="webpace_sl_effects" id="webpace_sl_effects">
											<option <?php if ( $row->webpace_sl_effects == '0' ) {
												echo 'selected';
											} ?> value="0"><?php _e('Video Gallery/Content-Popup', 'gallery-video');?>
											</option>
											<option <?php if ( $row->webpace_sl_effects == '1' ) {
												echo 'selected';
											} ?> value="1"><?php _e('Content Video Slider', 'gallery-video');?>
											</option>
											<option <?php if ( $row->webpace_sl_effects == '5' ) {
												echo 'selected';
											} ?> value="5"><?php _e('Lightbox-Video Gallery', 'gallery-video');?>
											</option>
											<option <?php if ( $row->webpace_sl_effects == '3' ) {
												echo 'selected';
											} ?> value="3"><?php _e('Video Slider', 'gallery-video');?>
											</option>
											<option <?php if ( $row->webpace_sl_effects == '4' ) {
												echo 'selected';
											} ?> value="4"><?php _e('Thumbnails View', 'gallery-video');?>
											</option>
											<option <?php if ( $row->webpace_sl_effects == '6' ) {
												echo 'selected';
											} ?> value="6"><?php _e('Justified', 'gallery-video');?>
											</option>
											<option <?php if ( $row->webpace_sl_effects == '7' ) {
												echo 'selected';
											} ?> value="7"><?php _e('Blog Style Gallery', 'gallery-video');?>
											</option>
                                            <option <?php if ( $row->webpace_sl_effects == '8' ) {
                                                echo 'selected';
                                            } ?> value="8">Playlist
                                            </option>
										</select>
									...
									<li id="videogallery-current-options-0"
									     class="videogallery-current-options <?php if ( $row->webpace_sl_effects == 0 ) {
										     echo ' active';
									     } ?>">
										<ul id="view4">
											<li>
									...
									<li id="videogallery-current-options-1"
									     class="videogallery-current-options <?php if ( $row->webpace_sl_effects == 1 ) {
										     echo ' active';
									     } ?>">
										<ul>
											<li>
									...
									<li id="videogallery-current-options-3"
									     class="videogallery-current-options <?php if ( $row->webpace_sl_effects == 3 ) {
										     echo ' active';
									     } ?>">
									...
									<li id="videogallery-current-options-4"
									     class="videogallery-current-options <?php if ( $row->webpace_sl_effects == 4 ) {
										     echo ' active';
									     } ?>">
										<ul id="view4">
											<li>
									...
									<li id="videogallery-current-options-5"
									     class="videogallery-current-options <?php if ( $row->webpace_sl_effects == 5 ) {
										     echo ' active';
									     } ?>">
										<ul id="view4">
											<li>
									...
									<li id="videogallery-current-options-6"
									     class="videogallery-current-options <?php if ( $row->webpace_sl_effects == 6 ) {
										     echo ' active';
									     } ?>">
										<ul id="view4">
											<li>
									...
									<li id="videogallery-current-options-7"
									     class="videogallery-current-options <?php if ( $row->webpace_sl_effects == 7 ) {
										     echo ' active';
									     } ?>">
										<ul id="view7">
											<li>
									...
									<li>
										...
										<a class="video-pro-link probuttonlink" href="https://webpace.net/video-gallery/" target="_blank"><span class="video-pro-icon"></span></a>
									</li>
								...
								<div class="inside">
									<ul>
										<li rel="tab-1" class="selected">
											...
											<textarea class="full"
											          readonly="readonly">[webpace_videogallery id="<?php echo $row->id; ?>"]</textarea>
										</li>
										<li rel="tab-2">
											...
											<textarea class="full" readonly="readonly">&lt;?php echo do_shortcode("[webpace_videogallery id='<?php echo $row->id; ?>']"); ?&gt;</textarea>
										</li>
									</ul>
								</div>
							</div>
...
<?php
require_once( GALLERY_VIDEO_TEMPLATES_PATH . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'webp-video-gallery-admin-video-add-view.php' );
require_once( GALLERY_VIDEO_TEMPLATES_PATH . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'webp-video-gallery-admin-video-edit-view.php' );
?>