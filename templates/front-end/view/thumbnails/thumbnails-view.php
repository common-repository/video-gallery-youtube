<section id="thumbwrapper<?php echo esc_attr($gallery_videoID); ?>" class="gallery-video-content"
    ...
    <ul id="webpace_videogallery" class="webpace_videogallery view-<?php echo esc_attr($view_slug); ?>">
        <li id="fullPreview"></li>
        <input type="hidden" id="total" value="<?php echo esc_attr($total); ?>"/>
        <?php foreach ($page_videos as $key => $row) { ?>
            <li class="webpace_big_li" data-id="<?php echo esc_attr($row->id); ?>">
                <?php
                ...
                if ($videourl[1] == 'youtube') {
                    ...
                    ?>
                    <a class="vyoutube webpace_videogallery_item group<?php echo $gallery_videoID; ?>"
                    ...
                    ?>
                    <a class="vvimeo webpace_videogallery_item group<?php echo $gallery_videoID; ?>"
    ...
    if ($a == 1 && $num < $total_videos) {
        ...
        <div class="paginate3">
            ...
            $pervpage = '';
            if ($page != 1) {
                $pervpage = '<a href= ' . $checkREQ . '=1><i class="icon-style3 webpaceicons-fast-backward" ></i></a>  
	                               <a href= ' . $checkREQ . '=' . ($page - 1) . '><i class="icon-style3 webpaceicons-chevron-left"></i></a> ';
            }
            $nextpage = '';
            if ($page != $total) {
                $nextpage = ' <a href= ' . $checkREQ . '=' . ($page + 1) . '><i class="icon-style3 webpaceicons-chevron-right"></i></a>  
                                   <a href= ' . $checkREQ . '=' . $total . '><i class="icon-style3 webpaceicons-fast-forward" ></i></a>';
            }
